...
{
    /**
     * Show the notification settings page with current user preferences.
     */
    public function settings()
    {
        ...
        return view('notification-settings', [
            'preferences' => $preferences->toFormArray(),
        ]);
    }
     ...
     */
    public function checkNewWork(Request $request)
    {
        try {
            $user = Auth::user();
            ...
            \Log::error('Error checking for new work: ' . $e->getMessage(), [
                'user_id' => Auth::id(),
    ...
    private function checkLabWork($user, $branchId, $lastCheckTime, $preferences)
    {
        ...
        if ($preferences->notify_lab_queue) {
            ...
            if (class_exists('App\Models\LabRequest')) {
    ...
    private function checkPharmacyWork($user, $branchId, $lastCheckTime, $preferences)
    {
        ...
        if ($preferences->notify_pharmacy_queue) {
            ...
            if ($preferences->notify_prescription_ready && class_exists('App\Models\Prescription')) {
    ...
    private function checkEmergencyAlerts($user, $branchId, $lastCheckTime, $preferences)
    {
        ...
        if ($preferences->notify_emergency_queue && class_exists('App\Models\EmergencyAlert')) {
