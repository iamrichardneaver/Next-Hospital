...
class TeleconsultationController extends Controller
{
    ...
    public function getJitsiConfig(Teleconsultation $teleconsultation): JsonResponse
    {
        try {
            ...
            // Authorize access (patient_id is patients.id, doctor_id is users.id)
            if ($isPatient) {
                $userPatient = $user->patient ?? Patient::where('user_id', $user->id)->first();
                if (!$userPatient || $teleconsultation->patient_id != $userPatient->id) {
                    return response()->json([
                        'success' => false,
            ...
            $config['teleconsultation_id'] = $teleconsultation->id;
            $config['meeting'] = [
