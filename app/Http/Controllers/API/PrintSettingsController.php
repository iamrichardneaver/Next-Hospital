...
class PrintSettingsController extends Controller
{
    ...
    /**
     * Quick print invoice with default settings
     */
    public function quickPrintInvoice(Request $request, $invoiceId)
    {
        try {
            $settings = session('print_settings', ['default_format' => 'a4']);
            $format = $settings['default_format'] ?? 'a4';
            $options = $this->billingPdfService->parseFormatOptions($format);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error printing invoice: ' . $e->getMessage()
            ], 500);
        }
    }

    /**
     * Quick print receipt with default settings
     */
    public function quickPrintReceipt(Request $request, $invoiceId)
    {
        try {
            $settings = session('print_settings', ['default_format' => 'a4']);
            $format = $settings['default_format'] ?? 'a4';
