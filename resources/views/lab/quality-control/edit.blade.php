...
@section('title', 'Edit Quality Control Record')

@section('content')
<div class="container-fluid">
    <div class="mb-4">
        <h1 class="h3 mb-1" style="color: #1e3a5f;">
            <i class="bi bi-pencil"></i> Edit Quality Control Record
        </h1>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('lab.quality-control.update', $qualityControl) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="parameter_id" class="form-select @error('parameter_id') is-invalid @enderror" required>
                            ...
                            @foreach($parameters as $param)
                                <option value="{{ $param->id }}" {{ ($qualityControl->parameter_id == $param->id || old('parameter_id') == $param->id) ? 'selected' : '' }}>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="qc_type" class="form-select @error('qc_type') is-invalid @enderror" required>
                            <option value="internal" {{ ($qualityControl->qc_type == 'internal' || old('qc_type') == 'internal') ? 'selected' : '' }}>Internal QC</option>
                            <option value="external" {{ ($qualityControl->qc_type == 'external' || old('qc_type') == 'external') ? 'selected' : '' }}>External QC</option>
                        </select>
                        @error('qc_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="qc_level" class="form-select @error('qc_level') is-invalid @enderror" required>
                            <option value="level_1" {{ ($qualityControl->qc_level == 'level_1' || old('qc_level') == 'level_1') ? 'selected' : '' }}>Level 1</option>
                            <option value="level_2" {{ ($qualityControl->qc_level == 'level_2' || old('qc_level') == 'level_2') ? 'selected' : '' }}>Level 2</option>
                            <option value="level_3" {{ ($qualityControl->qc_level == 'level_3' || old('qc_level') == 'level_3') ? 'selected' : '' }}>Level 3</option>
                        </select>
                        @error('qc_level')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="qc_material" class="form-control @error('qc_material') is-invalid @enderror" 
                               value="{{ old('qc_material', $qualityControl->qc_material) }}" required placeholder="e.g., Control Serum">
                        @error('qc_material')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="lot_number" class="form-control @error('lot_number') is-invalid @enderror" 
                               value="{{ old('lot_number', $qualityControl->lot_number) }}" required>
                        @error('lot_number')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="date" name="expiry_date" class="form-control @error('expiry_date') is-invalid @enderror" 
                               value="{{ old('expiry_date', $qualityControl->expiry_date ? $qualityControl->expiry_date->format('Y-m-d') : '') }}" required>
                        @error('expiry_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.0001" name="target_value" 
                               ...
                               value="{{ old('target_value', $qualityControl->target_value) }}" required>
                        @error('target_value')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.0001" name="measured_value" 
                               ...
                               value="{{ old('measured_value', $qualityControl->measured_value) }}" required>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.0001" name="acceptable_range_low" 
                               ...
                               value="{{ old('acceptable_range_low', $qualityControl->acceptable_range_low) }}" required>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.0001" name="acceptable_range_high" 
                               ...
                               value="{{ old('acceptable_range_high', $qualityControl->acceptable_range_high) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" 
                              rows="3" placeholder="Any additional notes or observations">{{ old('notes', $qualityControl->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check-circle"></i> Update QC Record
                    </button>
