...
@section('title', 'Receive Lab Goods')

@section('content')
<div class="container-fluid">
    <div class="mb-4">
        <h1 class="h3" style="color: #1e3a5f;"><i class="bi bi-box-arrow-in-down"></i> Receive Lab Goods — {{ $order->po_number }}</h1>
        <p class="text-secondary">Received quantities will be added to laboratory inventory at {{ $order->branch?->name }}</p>
    </div>

    @if(session('error'))<div class="alert alert-danger">{{ session('error') }}</div>@endif

    <form method="POST" action="{{ route('lab.purchases.receive.store', $order) }}">
        @csrf
        <div class="card mb-4">
        ...
        <a href="{{ route('lab.purchases.show', $order) }}" class="btn btn-link">Cancel</a>
    </form>
</div>
@endsection
